...
void re_exec_child(int new_socket){
    ...
    if(c_pid<0) { // Error case - negative value
        perror("Fork failed");
        exit(EXIT_FAILURE);
    } else if(c_pid == 0) { // Child process case - zero value
        char socket_add[12];
    ...
    } else if (c_pid > 0) { // Parent process case - positive value
...
int main(int argc, char const *argv[]) {
    ...
    } else { // Runs when 2 command line arguments are passed. It will run when socket argument is passed
        struct passwd *pw;
